...
def add_gaussian_noise(image: np.ndarray, mean: float = 0.0, sigma: float = 10.0) -> np.ndarray:
    # Convert to float to avoid overflow/underflow while adding noise
    img = image.astype(np.float32)
    noise = np.random.normal(loc=mean, scale=sigma, size=img.shape).astype(np.float32)
    noisy = img + noise
...
def add_salt_and_pepper_noise(image: np.ndarray, salt_prob: float = 0.01, pepper_prob: float = 0.01) -> np.ndarray:
    ...
    # Random mask over spatial dimensions (H, W)
    rnd = np.random.rand(*image.shape[:2])

    # Pepper: set to 0
    pepper_mask = rnd < pepper_prob
    noisy[pepper_mask] = 0

    # Salt: set to 255
    salt_mask = rnd > (1.0 - salt_prob)
    noisy[salt_mask] = 255
...
def add_poisson_noise(image: np.ndarray) -> np.ndarray:
    # Poisson expects non-negative lambda; image intensities are OK.
    img = image.astype(np.float32)

    noisy = np.random.poisson(lam=img).astype(np.float32)
    noisy = np.clip(noisy, 0, 255)
    return noisy.astype(np.uint8)
...
def add_uniform_noise(image: np.ndarray, low: float = -20.0, high: float = 20.0) -> np.ndarray:
    img = image.astype(np.float32)
    noise = np.random.uniform(low=low, high=high, size=img.shape).astype(np.float32)
    noisy = img + noise
    noisy = np.clip(noisy, 0, 255)
    return noisy.astype(np.uint8)
